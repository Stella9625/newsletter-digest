"""Claude 适配器：调用 Anthropic API 实现摘要、翻译、标签提取和日报生成"""
...
class ClaudeProvider(LLMProvider):
    ...
    def summarize(self, text: str, title: str = "") -> str:
        ...
        return self._call(HAIKU_MODEL, system, user_msg, "summarize")
    ...
    def extract_tags(self, text: str, title: str = "") -> list[str]:
        ...
        result = self._call(HAIKU_MODEL, system, user_msg, "extract_tags")

        # 解析 JSON 数组
        try:
            # 处理可能包含 markdown code block 的情况
    ...
    def translate(self, text: str, title: str = "") -> str:
        """用 Sonnet 全文翻译，输出双语对照 HTML"""
        system = (
            "你是一个专业的英中翻译。请将以下英文科技文章翻译为双语对照格式的 HTML。\n\n"
            "**输出格式要求（严格遵守）：**\n"
            "- 逐段翻译：每一段英文原文后紧跟其中文翻译\n"
            "- 英文原文直接展示，用 <div style=\"color:#9a9ea6;font-size:14px;line-height:1.7;margin:20px 0 6px;\"> 包裹。注意：原文中如果有引用、列表、代码等格式，需要在 div 内部用对应 HTML 标签正确渲染，不要丢弃原文格式。\n"
            "- 中文翻译用 <p style=\"color:#2c2c2e;margin:4px 0 24px;line-height:1.8;font-size:15px;\"> 包裹，与下一段英文原文之间通过较大的 margin 留白分隔。\n"
        ...
        return self._call(SONNET_MODEL, system, user_msg, "translate")

    def extract_quotes_and_tone(self, text: str, title: str = "") -> dict:
        """用 Haiku 提取金句 + 检测语气 + 翻译标题（一次调用）"""
        system = (
        ...
        result = self._call(HAIKU_MODEL, system, user_msg, "quotes_and_tone")
    ...
    def generate_daily_report(self, articles_data: list[dict]) -> str:
        ...
        return self._call(SONNET_MODEL, system, user_msg, "daily_report")
